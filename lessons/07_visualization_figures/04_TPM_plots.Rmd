...
avg_cols <- grep("_avg", names(Neurod1_tpm), value = TRUE)

sd_cols <- grep("_sd", names(Neurod1_tpm), value = TRUE)

# This is another way to "melt" or pivot longer
plot_df <- data.frame(
  time = as.numeric(gsub("_avg", "", avg_cols)),
  avg = unlist(Neurod1_tpm[avg_cols]),
  sd = unlist(Neurod1_tpm[sd_cols])
)
